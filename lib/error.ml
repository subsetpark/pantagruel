...
let format_type_error err =
  ...
  | TypeMismatch (expected, got, loc) ->
      fmt loc (Printf.sprintf "Type mismatch: expected %s, got %s"
        (Types.format_ty expected) (Types.format_ty got))
  | ArityMismatch (expected, got, loc) ->
      fmt loc (Printf.sprintf "Wrong number of arguments: expected %d, got %d"
        expected got)
  | NotAFunction (ty, loc) ->
      fmt loc (Printf.sprintf "Cannot call %s: not a function"
        (Types.format_ty ty))
  | NotAList (ty, loc) ->
  ...
  | NotNumeric (ty, loc) ->
      fmt loc (Printf.sprintf "Expected numeric type, got %s"
        (Types.format_ty ty))
  | ExpectedBool (ty, loc) ->
  ...
  | PrimeOutsideVoidContext (name, loc) ->
      fmt loc (Printf.sprintf "Primed '%s' only valid in void procedure context"
        name)
  | VoidProcInExpression (name, loc) ->
      fmt loc (Printf.sprintf "Void procedure '%s' cannot be used in expression"
        name)
  | OverrideRequiresArity1 (name, arity, loc) ->
      fmt loc (Printf.sprintf
        "Override requires arity-1 procedure, '%s' has arity %d" name arity)
  | ProjectionOutOfBounds (idx, len, loc) ->
      fmt loc (Printf.sprintf "Projection .%d out of bounds for %d-tuple"
        idx len)
...
let format_collect_error err =
  ...
  | DuplicateDomain (name, loc, first_loc) ->
      fmt loc (Printf.sprintf "Duplicate domain '%s' (first defined at %s)"
        name (format_loc first_loc))
  | DuplicateProc (name, loc, first_loc) ->
      fmt loc (Printf.sprintf "Duplicate procedure '%s' (first defined at %s)"
        name (format_loc first_loc))
