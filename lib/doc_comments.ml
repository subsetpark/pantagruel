...
let add line col docs adjacent =
  if docs <> [] then
    Hashtbl.add doc_map (Doc_comment_positions.key line col) (docs, adjacent)

(** Look up docs at a position. Returns (groups, adjacent). *)
let get line col =
  match Hashtbl.find_opt doc_map (Doc_comment_positions.key line col) with
  | Some result -> result
  | None -> ([], true)

(** Look up docs by Lexing.position *)
let get_at_pos pos =
  let line, col = Doc_comment_positions.key_at_pos pos in
  get line col
