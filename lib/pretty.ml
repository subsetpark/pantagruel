...
let rec pp_type_expr fmt = function
  ...
  | TProduct ts ->
      pp_print_list ~pp_sep:(fun fmt () -> fprintf fmt " * ") pp_type_expr_atom fmt ts
  | TSum ts ->
      pp_print_list ~pp_sep:(fun fmt () -> fprintf fmt " + ") pp_type_expr_product fmt ts
...
let rec pp_expr fmt = function
  | EForall (params, guards, body) ->
      fprintf fmt "all %a | %a" pp_quant_params_guards (params, guards) pp_expr body
  | EExists (params, guards, body) ->
      fprintf fmt "some %a | %a" pp_quant_params_guards (params, guards) pp_expr body
...
and pp_quant_params_guards fmt (params, guards) =
  ...
  pp_print_list ~pp_sep:pp_sep_comma (fun fmt -> function
...
and pp_primary fmt = function
  | EApp (f, args) when args <> [] ->
      fprintf fmt "%a %a" pp_atom f
        (pp_print_list ~pp_sep:pp_sep_space pp_atom) args
  | e -> pp_atom fmt e
...
let format_chapter ?(width=80) fmt chapter =
  ...
  List.iter (fun decl ->
    ...
    if has_doc && !prev_had_doc then
      fprintf fmt "@\n";
    if has_doc then
      pp_docs_wrapped width fmt decl.doc;
  ...
  List.iter (fun prop ->
    if prop.doc <> [] then
      pp_docs_wrapped width fmt prop.doc;
    fprintf fmt "%a.@\n" pp_expr prop.value
  ) chapter.body
