(** Normal form transformation for Pantagruel documents.
    ...
    dependency levels, and propositions are placed in the earliest valid
    chapter based on their dependencies.
    ...
    Algorithm:
    1. Build dependency graph of declarations (based on type references)
    2. Topologically sort declarations into levels
    3. Each level becomes a chapter
    4. Propositions are placed in the earliest body where all dependencies are visible
    5. Void procedures and their tied propositions stay together
*)
...
type void_unit = {
  void_proc: decl_info;
  tied_props: expr located list;  (** Props using primed exprs or void proc params *)
}
...
let symbols_in_expr expr =
  ...
  let rec go = function
    ...
    | EOverride (name, pairs) ->
        ...
        List.iter (fun (k, v) -> go k; go v) pairs
    | ETuple es -> List.iter go es
    | EProj (e, _) -> go e
    | EBinop (_, e1, e2) -> go e1; go e2
    | EUnop (_, e) -> go e
    | EForall (params, guards, body) | EExists (params, guards, body) ->
        ...
        List.iter (function
          | GParam p -> types := StringSet.union !types (types_in_type_expr p.param_type)
          | GIn (_, e) -> go e  (* x in xs - the list expression has dependencies *)
          | GExpr e -> go e
        ) guards;
        go body
  in
...
(** Extract symbols from a declaration's dependencies (not the symbol it defines) *)
let symbols_in_decl_deps (decl : declaration) =
  ...
  (match decl with
   ...
   | DeclProc { params; guards; return_type; _ } ->
       ...
       Option.iter (fun te ->
         types := StringSet.union !types (types_in_type_expr te)
       ) return_type);
  (!types, !terms)
...
let rec uses_primed = function
  ...
  | EOverride (_, pairs) -> List.exists (fun (k, v) -> uses_primed k || uses_primed v) pairs
...
let void_proc_params (chapter : chapter) : StringSet.t =
  let params = List.find_map (fun decl ->
...
let compute_levels (decls : decl_info list) : int =
  ...
  while !changed do
    ...
    List.iter (fun d ->
      ...
      let max_dep_level = StringSet.fold (fun dep_name acc ->
        match Hashtbl.find_opt by_name dep_name with
        | Some dep -> max acc dep.level
        | None -> acc  (* Built-in or unknown, level 0 *)
      ) d.dependencies 0 in
      let new_level = if StringSet.is_empty d.dependencies then 0 else max_dep_level + 1 in
...
(** Find the earliest chapter where a proposition can be placed.
    Based on its dependencies - needs all deps visible in that body. *)
let earliest_chapter_for_prop decl_levels prop =
  ...
  (* Filter out builtins *)
  let deps = StringSet.filter (fun n -> not (StringSet.mem n builtin_types)) all_deps in
  (* Find max level among dependencies *)
  let max_level = StringSet.fold (fun name acc ->
...
let normalize (doc : document) : document =
  if doc.chapters = [] then doc
    ...
    let void_units = List.map (fun vd ->
      (* Find the original chapter this void proc came from *)
      let orig_chapter = List.find (fun ch ->
        List.exists (fun d -> decl_name d.value = vd.name) ch.head
      ) doc.chapters in
      let params = void_proc_params orig_chapter in
      let tied = List.filter (fun p ->
        uses_primed p.value || prop_uses_params params p.value
      ) orig_chapter.body in
    ...
    let independent_props =
      List.concat_map (fun chapter ->
        ...
        List.filter (fun p ->
          not (uses_primed p.value) && not (prop_uses_params params p.value)
        ) chapter.body
      ) doc.chapters in
    ...
    List.iter (fun d ->
      decl_assignments.(d.level) <- d :: decl_assignments.(d.level)
    ) non_void_decls;
    ...
    let sorted_void_units = List.sort
    ...
    List.iter (fun vu ->
      ...
      vu.void_proc.level <- target;  (* Update the level *)
    ...
    let decl_assignments =
      if num_chapters > Array.length decl_assignments then
        Array.init num_chapters (fun i ->
          if i < Array.length decl_assignments then decl_assignments.(i) else []
        )
    ...
    let new_chapters = Array.to_list (Array.mapi (fun level _ ->
      ...
      let sorted_decls = List.sort (fun a b -> String.compare a.name b.name) non_void @ void in
