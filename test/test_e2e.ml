...
(** Test that pretty-printed output matches stored snapshot *)
let test_snapshot_pretty name () =
  with_sample_dir (fun dir ->
      let path = Filename.concat dir name in
      let doc = parse_file path in
      let buf = Buffer.create 1024 in
      ...
      let snap_path =
        ...
        | None -> None
        | Some sd ->
            let p = Filename.concat sd ("pretty/" ^ base ^ ".pant") in
            if Sys.file_exists p then Some p else None
      in
      match snap_path with
      | Some p ->
          let expected = read_file p in
          check string (name ^ " pretty snapshot") expected actual
      | None -> ())

(** Test that markdown output matches stored snapshot *)
let test_snapshot_markdown name () =
  with_sample_dir (fun dir ->
      let path = Filename.concat dir name in
      let doc = parse_file path in
      let mod_name = Option.value ~default:"" doc.module_name in
      ...
      | Ok env -> (
          ...
          let snap_path =
            ...
            | None -> None
            | Some sd ->
                let p = Filename.concat sd ("markdown/" ^ base ^ ".md") in
                if Sys.file_exists p then Some p else None
          in
          match snap_path with
          | Some p ->
              let expected = read_file p in
              check string (name ^ " markdown snapshot") expected actual
          | None -> ()))
  ...
  if samples = [] then
    ...
    run "E2E"
      [
        ...
        ( "snapshot_pretty",
          List.map
            (fun name -> test_case name `Quick (test_snapshot_pretty name))
            samples );
        ( "snapshot_markdown",
          List.map
            (fun name -> test_case name `Quick (test_snapshot_markdown name))
            samples );
      ]
