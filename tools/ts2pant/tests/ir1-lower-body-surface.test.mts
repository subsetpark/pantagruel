...
it("generated inputs exercise the ir1-lower-body exported surface", () => {
  fc.assert(
    fc.property(fc.constantFrom("Account_balance", "Account_limit"), (prop) => {
      ...
      assert.equal(LB.lowerL1BodyToSsaProps(stmt, [], options).programs.length, 1);
      assert.equal(LB.lowerScalarL1BodyToSsaResult(stmt, options).programs.length, 1);
      assert.equal(LB.lowerCollectionL1BodyToSsaResult(stmt, options).programs.length, 1);
    }),
  );
});
